function fig_all  = plotIntersectionRun(run_name, varargin)
    ...
    % only the nb vehicle using game agent actually has the role
    plot(time_sim(1:end-1), categorical(role_sim, {'Follower', 'Leader'}), nb_colors{1});
    if game_param.role_update_type == 2 || game_param.role_update_type == 4
        plot(time_sim(1:end-1), categorical(role_sim_next, {'Follower', 'Leader'}), nb_colors{2});
    end

    iter = iter + 1;
    ax(iter) = subplot(num_rows, num_cols, iter);hold on;box on;grid on;
    % plot(time_sim(1:end-1), trace_prob_all(1, :), 'go-');
    % plot(time_sim(1:end-1), trace_prob_all(2, :), 'ms:');
    % legend({"$p_{\rm l}$", "$p_{\rm f}$"}, "Interpreter", "Latex")
    % ylabel("$P$(Role)", 'Rotation',0);
    % ylim([0.0, 1.0]);
    ...
    if ~options.publish
        ylabel("$p$", 'Rotation',0);
        legend({"blue (nb) thinks red as Leader", ...
                "red (eb) thinks blue as Leader"}, "Interpreter", "Latex");
    else
        % plot legend line
    ...
    if ~options.publish
        iter = iter + 1;
        ax(iter) = subplot(num_rows, num_cols, iter);hold on;box on;grid on;
        plot(time_sim(1:end-1), optimal_chance_constraint_range(1, :), 'bo-');
        plot(time_sim(1:end-1), optimal_chance_constraint_range(2, :), 'rs-');
        plot(time_sim(1:end-1), chance_constraint_range(1, :), 'cd:');
        plot(time_sim(1:end-1), chance_constraint_range(2, :), 'm>:');
        plot(time_sim(1:end-1), time_sim(1:end-1) *0 + 1 - mpc_param.epsilon, "k--")
        chance_level = sprintf("$1-\\epsilon$ = %.2f", 1 - mpc_param.epsilon);
        legend({"$P_{\rm optimal,~max}$", "$P_{\rm optimal,~min}$", ...
                "$P_{\rm all,~min-max}$", "$P_{\rm all,~min-min}$", ...
                chance_level}, "Interpreter", "Latex");
        ylim([0.0, 1.0]);
        ylabel("Chance", 'Rotation',0);

        iter = iter + 1;
        ax(iter) = subplot(num_rows, num_cols, iter);hold on;box on;grid on;
        % only the nb vehicle using game agent actually has the role
        plot(time_sim(1:end - 1), reshape(role_trans_prob(1, 1, :), [1, steps]), nb_colors{1});
        plot(time_sim(1:end - 1), reshape(role_trans_prob(2, 2, :), [1, steps]), nb_colors{2});
        ylim([0.0, 1.0]);
    ...
    if ~options.publish
        legend({"game blue nb", "MPC red eb"});
        ylabel("$v {[\rm m/s]}$", 'Rotation',0);
    else
        ylim([0.0, 10.0]);
    end
    ...
    if ~options.publish
        iter = iter + 1;
        ax(iter) = subplot(num_rows, num_cols, iter);hold on;box on;grid on;
        % nb game first, then eb second in the logging
    ...
    switch flag
        case 1
            fprintf("eastbound (red mpc) vehicle arrived first. \n");
        case 2
            fprintf("northbound (blue game) vehicle arrived first. \n");
        case 3
